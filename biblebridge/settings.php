...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['base_url'])) {
    $newBase = rtrim(trim($_POST['base_url'] ?? ''), '/');
    // Must be empty (root install) or start with /
    if ($newBase === '' || preg_match('#^/[a-zA-Z0-9/_-]*$#', $newBase)) {
        $localConfig['base_url'] = $newBase;
        $export = var_export($localConfig, true);
        if (@file_put_contents($configFile, "<?php\nreturn " . $export . ";\n")) {
            // Also patch .htaccess RewriteBase to match
            $htaccess = __DIR__ . '/.htaccess';
            if (is_writable($htaccess)) {
                $contents = file_get_contents($htaccess);
                $rewriteBase = ($newBase === '') ? '/' : $newBase . '/';
                $contents = preg_replace('/^RewriteBase\s+\S+/m', 'RewriteBase ' . $rewriteBase, $contents);
                file_put_contents($htaccess, $contents);
            }
            $bbBaseUrl = $newBase;
            $message = 'Base URL updated. Navigation links will now use: <code>' . htmlspecialchars($newBase ?: '/') . '</code>';
            $messageType = 'success';
        } else {
            $message = 'Could not write config file. Check folder permissions.';
            $messageType = 'error';
        ...
        $message = 'Invalid base URL. Must be empty (for root installs) or start with / (e.g. /bible).';
        $messageType = 'error';
    }
}
...
// Handle theme preset save
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['theme_preset'])) {
    $preset = $_POST['theme_preset'];
    if (!isset(BB_THEME_PRESETS[$preset])) $preset = 'default';
    $localConfig['theme_preset'] = $preset;
    $export = var_export($localConfig, true);
    if (@file_put_contents($configFile, "<?php\nreturn " . $export . ";\n")) {
        $bbThemePreset = $preset;
        // Clear server page cache so cached HTML picks up the new theme URL
...
<head>
    ...
    <link rel="stylesheet" href="<?= $bbBaseUrl ?>/theme.php?p=<?= urlencode($bbThemePreset) ?>&amp;v=<?= BB_VERSION ?>">
    ...
    <style>
        ...
        [data-theme="dark"] .msg-success { background: #052e16; border-color: #166534; color: #4ade80; }
        [data-theme="dark"] .msg-error { background: #2d0a0a; border-color: #b91c1c; color: #f87171; }
...
<main class="settings-main">
    ...
    <!-- Base URL -->
    <div class="settings-section">
        <div class="settings-section-title">Base URL</div>
        <p style="font-size:0.83rem;color:var(--text-secondary);margin:0 0 0.85rem;">
            The URL path where BibleBridge is installed. Leave empty if BibleBridge is at your site root.
            Set to <code>/bible</code> (no trailing slash) if it's at <code>example.com/bible/</code>.
            If your navigation links go to <code>/read/</code> instead of <code>/bible/read/</code>, set this to fix it.
        </p>
        <form method="post">
            <div class="settings-row">
                <input type="text" name="base_url" class="settings-input"
                    placeholder="e.g. /bible  (leave empty for root installs)"
                    value="<?= htmlspecialchars($localConfig['base_url'] ?? $bbBaseUrl) ?>">
                <button type="submit" class="btn-save">Save</button>
            </div>
        </form>
        <p style="font-size:0.78rem;color:var(--text-muted);margin:0.5rem 0 0;">
            Currently detected as: <code><?= htmlspecialchars($bbBaseUrl ?: '(empty — root install)') ?></code>
        </p>
    </div>
    ...
    <div class="settings-section">
        ...
        <p style="font-size:0.83rem;color:var(--text-secondary);margin:0 0 1.25rem;">
            Choose a color theme for your reader. Changes take effect immediately for all visitors.
        </p>
        <?php $activePreset = $bbThemePreset ?? 'default'; ?>
        <div style="display:grid;grid-template-columns:repeat(auto-fill,minmax(130px,1fr));gap:0.75rem;">
        <?php foreach (BB_THEME_PRESETS as $key => $preset): ?>
            <?php $active = ($activePreset === $key); ?>
            <form method="post" style="margin:0;">
                <input type="hidden" name="theme_preset" value="<?= $key ?>">
                <button type="submit" style="
                    width:100%;padding:0.75rem 0.5rem;border-radius:8px;cursor:pointer;
                    border:2px solid <?= $active ? 'var(--accent)' : 'var(--border-light)' ?>;
                    background:<?= $active ? 'var(--bg-hover)' : 'var(--bg-card)' ?>;
                    font-family:inherit;text-align:center;transition:border-color 0.15s;">
                    <div style="display:flex;justify-content:center;gap:4px;margin-bottom:0.5rem;">
                        <span style="display:inline-block;width:28px;height:28px;border-radius:50%;background:<?= $preset['accent'] ?>;border:1px solid rgba(0,0,0,0.1);"></span>
                        <span style="display:inline-block;width:28px;height:28px;border-radius:50%;background:<?= $preset['sidebar'] ?>;border:1px solid rgba(0,0,0,0.1);"></span>
                    </div>
                    <div style="font-size:0.8rem;font-weight:<?= $active ? '700' : '500' ?>;color:var(--text-primary);"><?= $preset['name'] ?></div>
                    <?php if ($active): ?>
                    <div style="font-size:0.7rem;color:var(--accent);margin-top:2px;">Active</div>
                    <?php endif; ?>
                </button>
            </form>
        <?php endforeach; ?>
...
function applyUpdate() {
    ...
    .catch(function(e) {
        ...
        if (e.name === 'AbortError') {
            progress.innerHTML = '<span style="color:#b91c1c;">Update timed out after 90 seconds. Your host may be too slow for one-click updates. Try the manual upload method instead.</span>';
        } else {
...
<?php if ($clearSwCache): ?>
// Clear SW page cache so theme change applies to offline-cached chapter pages
