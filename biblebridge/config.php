...
// Stored value from setup wins; auto-detection is the fallback for
// older installs or servers where DOCUMENT_ROOT is unreliable.
// -----------------------------------------------------------
if (isset($bbInstall['base_url'])) {
    $bbBaseUrl = $bbInstall['base_url'];
} else {
...
// -----------------------------------------------------------
// Theme presets
// accent / dark_accent: pre-verified WCAG AA against white and dark bg
// sidebar: affects left nav + header only — no page background changes
// -----------------------------------------------------------

define('BB_THEME_PRESETS', [
    'default' => ['name' => 'Default', 'accent' => '#9e7640', 'dark_accent' => '#c49840', 'sidebar' => '#1a1512'],
    'forest'  => ['name' => 'Forest',  'accent' => '#236b42', 'dark_accent' => '#3da866', 'sidebar' => '#163d25'],
    'ocean'   => ['name' => 'Ocean',   'accent' => '#1a7a8a', 'dark_accent' => '#2ab0c8', 'sidebar' => '#0d3545'],
    'royal'   => ['name' => 'Royal',   'accent' => '#6b3fa0', 'dark_accent' => '#9b6fd4', 'sidebar' => '#251045'],
    'crimson' => ['name' => 'Crimson', 'accent' => '#c41e34', 'dark_accent' => '#e8304a', 'sidebar' => '#3a0e15'],
    'slate'   => ['name' => 'Slate',   'accent' => '#2e5fa3', 'dark_accent' => '#5a8fd4', 'sidebar' => '#0f2040'],
    'ember'   => ['name' => 'Ember',   'accent' => '#e8640a', 'dark_accent' => '#f07828', 'sidebar' => '#5a1f14'],
    'olive'   => ['name' => 'Olive',   'accent' => '#7a8a18', 'dark_accent' => '#a8bc30', 'sidebar' => '#202808'],
    'rose'    => ['name' => 'Rose',    'accent' => '#a83868', 'dark_accent' => '#d060a0', 'sidebar' => '#3a0d28'],
]);

function _bbHexToRgb(string $hex): array {
    $hex = ltrim($hex, '#');
    if (strlen($hex) === 3) $hex = $hex[0].$hex[0].$hex[1].$hex[1].$hex[2].$hex[2];
    return ['r' => hexdec(substr($hex,0,2)), 'g' => hexdec(substr($hex,2,2)), 'b' => hexdec(substr($hex,4,2))];
}
function _bbRgbToHex(int $r, int $g, int $b): string {
    return '#' . sprintf('%02x%02x%02x', $r, $g, $b);
}
function _bbHue2Rgb(float $p, float $q, float $t): float {
...
function _bbHexToHsl(string $hex): array {
    ['r' => $r, 'g' => $g, 'b' => $b] = _bbHexToRgb($hex);
    $r /= 255; $g /= 255; $b /= 255;
...
function _bbHslToHex(float $h, float $s, float $l): string {
    $h /= 360; $s /= 100; $l /= 100;
    if ($s == 0.0) { $v = (int)round($l * 255); return _bbRgbToHex($v,$v,$v); }
    $q = $l < 0.5 ? $l*(1+$s) : $l+$s-$l*$s;
    $p = 2*$l - $q;
    return _bbRgbToHex(
        (int)round(_bbHue2Rgb($p,$q,$h+1/3)*255),
        (int)round(_bbHue2Rgb($p,$q,$h)*255),
        (int)round(_bbHue2Rgb($p,$q,$h-1/3)*255)
    );
}
function _bbAdjustL(string $hex, float $delta): string {
    $hsl = _bbHexToHsl($hex);
    return _bbHslToHex($hsl['h'], $hsl['s'], max(0, min(100, $hsl['l'] + $delta)));
}
function _bbTintWhite(string $hex, float $pct): string {
    ['r'=>$r,'g'=>$g,'b'=>$b] = _bbHexToRgb($hex); $f = $pct/100;
    return _bbRgbToHex((int)round($r+$f*(255-$r)), (int)round($g+$f*(255-$g)), (int)round($b+$f*(255-$b)));
}

function bbGetThemeCssRules(string $preset): string {
    $presets = BB_THEME_PRESETS;
    if ($preset === 'default' || !isset($presets[$preset])) return '';
    $p = $presets[$preset];

    $a = $p['accent']; $da = $p['dark_accent']; $s = $p['sidebar'];
    ['r'=>$r,'g'=>$g,'b'=>$b]       = _bbHexToRgb($a);
    ['r'=>$dr,'g'=>$dg,'b'=>$db]    = _bbHexToRgb($da);

    $vars = [
        "--accent:$a",
        "--accent-hover:" . _bbAdjustL($a, -12),
        "--bg-hover:rgba($r,$g,$b,0.07)",
        "--highlight-bg:" . _bbTintWhite($a, 92),
        "--highlight-border:$a",
        "--search-em-bg:" . _bbTintWhite($a, 90),
        "--bg-sidebar:$s",
        "--brand-dark:$s",
        "--brand-mid:" . _bbAdjustL($s, 7),
    ];
    $darkVars = [
        "--accent:$da",
        "--accent-hover:" . _bbAdjustL($da, -12),
        "--bg-hover:rgba($dr,$dg,$db,0.08)",
        "--highlight-bg:rgba($dr,$dg,$db,0.15)",
        "--highlight-border:$da",
        "--search-em-bg:rgba($dr,$dg,$db,0.12)",
    ];

    return ":root{" . implode(';', $vars) . "}\n"
         . "[data-theme=\"dark\"]{" . implode(';', $darkVars) . "}\n";
}

$bbThemePreset = $bbInstall['theme_preset'] ?? 'default';
