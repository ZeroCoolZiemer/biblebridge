...
(function () {
    ...
    // Base URL for this BibleBridge install (empty string = site root).
    // Set by the PHP template as BB_BASE_URL; falls back to '' for CDN/holybible.dev usage.
    var _bbBase = (typeof BB_BASE_URL !== 'undefined') ? BB_BASE_URL : '';
    ...
    document.querySelectorAll('.sidebar-book-btn').forEach(function (btn) {
        btn.addEventListener('click', function () {
            ...
            window.location.href = _bbBase + '/read/' + bookSlug + '/1' + v;
        });
    });
    ...
    function loadXrefs(reference) {
        ...
        fetch(_bbBase + '/reader/xref.php?reference=' + encodeURIComponent(reference) + '&v=' + encodeURIComponent(v))
            .then(function (r) {
                if (r.status === 429) {
    ...
    function refToUrl(reference) {
        ...
        var verse   = m[3];
        var v = (typeof READER_VERSION !== 'undefined' && READER_VERSION !== 'kjv') ? '&v=' + READER_VERSION : '';
        return _bbBase + '/read/' + book + '/' + chapter + (verse ? '?verse=' + verse + v : (v ? '?' + v.slice(1) : ''));
    }
    ...
    function openHighlightBrowser() {
        ...
        Object.keys(allKeys).forEach(function (k) {
            ...
            byBook[bookSlug].items.push({
                key: k,
                ref: bookTitle + ' ' + ch + ':' + vs,
                url: _bbBase + '/read/' + bookSlug + '/' + ch + '/' + vs,
                color: hls[k] || null,
                note: notes[k] || null,
                ch: parseInt(ch),
                vs: parseInt(vs)
    ...
    document.querySelectorAll('.reader-search-form').forEach(function (form) {
        ...
        input.addEventListener('input', function () {
            ...
            debounceTimer = setTimeout(function () {
                ...
                var url = _bbBase + '/reader/search-suggest.php?q=' + encodeURIComponent(q);
    ...
    function syncPull(cb) {
        ...
        if (!code) return;
        fetch(_bbBase + '/reader/sync.php?code=' + encodeURIComponent(code))
            ...
            document.getElementById('syncGenBtn').addEventListener('click', function() {
                ...
                fetch(_bbBase + '/reader/sync.php', {
                    ...
                    if (r.status === 403) {
                        ...
                        setSyncCode(newCode);
                        return fetch(_bbBase + '/reader/sync.php', {
                            method: 'POST',
                            headers: {'Content-Type': 'application/json'},
